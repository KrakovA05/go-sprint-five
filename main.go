...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	ms := r.meanSpeed()
	calc := ((CaloriesMeanSpeedMultiplier*ms + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return calc
}
...
func (w Walking) Calories() float64 {
	...
	calc := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(SpeedInMS, 2)/HeightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return calc
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	ms := float64(s.meanSpeed())
	return (ms + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	res := InfoMessage{
		...
		Duration:     s.Duration,
		Distance:     float64(s.LengthPool) * float64(s.CountPool) / MInKm, // расстояние на основе длины и количества дорожек
		Speed:        s.meanSpeed(),                                        // расчет средней скорости для плавания
		Calories:     s.Calories(),                                         // расчет калорий для плавания
	}
	return res
}
